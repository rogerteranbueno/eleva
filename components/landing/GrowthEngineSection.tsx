  ...
  return (
    <div className="flex flex-col items-center text-center gap-2 flex-1 min-w-0">
      ...
      <div>
        ...
        <p className="text-[10px] text-white/50 mt-0.5 leading-snug">{sub}</p>
      </div>
    </div>
  )
}
...
function Detail({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-t border-white/8 first:border-0">
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <p className="text-[11px] text-white/50 leading-relaxed pb-3 pr-4">{children}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    <section className="relative py-24 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-6 space-y-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="text-5xl sm:text-6xl font-black text-white leading-[1.05] mb-5">
            {c.title}
          </h2>
          <p className="text-white/65 text-lg leading-relaxed max-w-2xl">{c.sub}</p>
        </motion.div>

        {/* ── Funnel flow ── */}
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="rounded-2xl border border-white/10 bg-white/3 p-6 sm:p-8">
          <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-1">{c.funnelTitle}</p>
          <p className="text-sm text-white/60 mb-8 max-w-lg">{c.funnelSub}</p>

          {/* Steps */}
          <div className="flex items-start gap-2 sm:gap-4">
            <FunnelStep icon={Users}      color="violet" n="3"   label={c.f1label} sub={c.f1sub} />
            <div className="flex items-center self-center pb-8 text-white/20 text-lg flex-shrink-0">→</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-white/40 mb-1">{c.gapTitle}</p>
              <p className="text-sm text-white/60 leading-relaxed">{c.gapSub}</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-white/50">{c.without}</span>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-white/50">{c.with}</span>
            ...
            <div className="rounded-xl bg-violet-500/8 border border-violet-500/20 p-4">
              ...
              <p className="text-[10px] text-white/35 mt-1.5 leading-snug">{c.gapNote}</p>
            </div>
          </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <Link href="/simulador#enrolamiento">
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="inline-flex items-center gap-2 px-8 py-4 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-sm font-bold transition-colors shadow-lg shadow-violet-600/25">
              {c.cta}
