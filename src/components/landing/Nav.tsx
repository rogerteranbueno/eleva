        ...
        <div className="hidden sm:flex items-center gap-0.5">
          {LINKS.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="text-[13px] text-muted-foreground hover:text-foreground transition-colors px-3 py-1.5 rounded-lg hover:bg-white/5"
            >
              {l.label}
            </Link>
          ))}
        ...
        {mobileOpen && (
          ...
          >
            <div className="px-6 py-4 space-y-1 bg-background/95">
              {[...LINKS, { href: "/login", label: "Entrar a mi centro" }].map((l) => (
                <Link
                  key={l.href}
                  href={l.href}
